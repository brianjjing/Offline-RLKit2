#!/bin/bash
# Multi-seed COMBO+DBG (DDPM/diffusion ELBO density guardian) on the Abiomed MCS digital
# twin. Sibling of combo_dbg_kde_mcs.sh / combo_dbg_vae_mcs.sh / combo_dbg_neuralode_mcs.sh /
...
#
# Note: --guardian-type is "diffusion" (this repo's enum), not "ddpm" -- GORMPO/LEQ call
# the same estimator "ddpm"; the filename here matches how it's referred to in chat.
#
#   SEEDS="44 43 42" bash run_example/bash_scr/mult_seed/combo_dbg_ddpm_mcs.sh
#
...
#
# Heads-up from the seed-42 all-estimators run: at PENALTY_COEF=0.4 this guardian saturates
# to max penalty (tanh ceiling) on ~100% of rollout samples -- confirmed against GORMPO's own
# tuning, not a bug, but a much harsher guardian than kde/vae/neuralode by design.
set -e
...
GUARDIAN_TYPE=diffusion
GUARDIAN=${GUARDIAN:-/public/gormpo/models/abiomed/diffusion}
# GORMPO_abiomed's own tuned coefficient for MCS (cormpo/config/real/mbpo_ddpm.yaml), read
# from LEQ2/bash_scr/leq_dbg/LEQ_DBG_MCS.sh.
PENALTY_COEF=${PENALTY_COEF:-0.4}
...
summary="${results_dir}/ddpm_multiseed_seed${seedtag}_${timestamp}.csv"
echo "seed,n_episodes,return,sem" > "$summary"

echo "============================================"
echo "Multi-Seed COMBO+DBG: Abiomed MCS (DDPM)"
echo "seeds: ${seeds[*]}  |  device: $DEVICE  |  extra: ${EXTRA_ARGS:-none}"
echo "summary: $REPO/$summary"
echo "============================================"

[ -f "${GUARDIAN}/checkpoint.pt" ] || {
    echo "ERROR: guardian not found: ${GUARDIAN}/checkpoint.pt" >&2; exit 1; }
...
for seed in "${seeds[@]}"; do
    ...
    # so this glob only ever matches diffusion/DDPM runs, further narrowed to this seed.
