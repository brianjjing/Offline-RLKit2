#!/bin/bash
# Multi-seed COMBO+DBG (NeuralODE density guardian) on the Abiomed MCS digital twin.
# Sibling of combo_dbg_kde_mcs.sh / combo_dbg_vae_mcs.sh / combo_dbg_ddpm_mcs.sh /
...
#
#   SEEDS="44 43 42" bash run_example/bash_scr/mult_seed/combo_dbg_neuralode_mcs.sh
#
...
#
# This guardian's dopri5 ODE solve dominates the runtime, unlike the other 3 estimators.
# Empirically (abiomed's 73-dim guardian input, rollout-batch-size 50000): chunk 4096 ->
# ~223s/rollout, chunk 8192 -> ~25s/rollout at ~4.3GiB. Default here is 8192; drop it if
# your card doesn't have that headroom, at a real time cost.
set -e
...
GUARDIAN_TYPE=neuralode
GUARDIAN=${GUARDIAN:-/public/gormpo/models/abiomed/neuralODE/}  # trailing slash required
# GORMPO_abiomed's own tuned coefficient for MCS (cormpo/config/real/mbpo_neuralode.yaml),
# read from LEQ2/bash_scr/leq_dbg/LEQ_DBG_MCS.sh.
PENALTY_COEF=${PENALTY_COEF:-0.2}
PENALTY_TYPE=${PENALTY_TYPE:-tanh}
GUARDIAN_CHUNK_SIZE=${GUARDIAN_CHUNK_SIZE:-8192}
...
summary="${results_dir}/neuralode_multiseed_seed${seedtag}_${timestamp}.csv"
echo "seed,n_episodes,return,sem" > "$summary"

echo "============================================"
echo "Multi-Seed COMBO+DBG: Abiomed MCS (NeuralODE)"
echo "seeds: ${seeds[*]}  |  device: $DEVICE  |  chunk: $GUARDIAN_CHUNK_SIZE  |  extra: ${EXTRA_ARGS:-none}"
echo "summary: $REPO/$summary"
echo "============================================"

[ -f "${GUARDIAN}_model.pt" ] && [ -f "${GUARDIAN}_metadata.pkl" ] || {
    echo "ERROR: guardian not found: ${GUARDIAN}_model.pt / _metadata.pkl" >&2; exit 1; }
...
for seed in "${seeds[@]}"; do
    ...
    python "$SCRIPT" --seed "$seed" --device "$DEVICE" \
        ...
        --penalty-coef "$PENALTY_COEF" --penalty-type "$PENALTY_TYPE" \
        --guardian-chunk-size "$GUARDIAN_CHUNK_SIZE" $EXTRA_ARGS

    # guardian_type is baked into the log dir (run_combo_dbg_mcs.py's make_log_dirs call),
    # so this glob only ever matches neuralode runs, further narrowed to this seed.
