#!/bin/bash
# Multi-seed COMBO+DBG (VAE density guardian) on the Abiomed MCS digital twin.
# Sibling of combo_dbg_kde_mcs.sh / combo_dbg_ddpm_mcs.sh / combo_dbg_neuralode_mcs.sh /
...
#
#   SEEDS="44 43 42" bash run_example/bash_scr/mult_seed/combo_dbg_vae_mcs.sh
#
...
GUARDIAN_TYPE=vae
GUARDIAN=${GUARDIAN:-/public/gormpo/models/abiomed/vae/abiomed_vae}
# GORMPO_abiomed's own tuned coefficient for MCS (cormpo/config/real/mbpo_vae.yaml), read
...
summary="${results_dir}/vae_multiseed_seed${seedtag}_${timestamp}.csv"
echo "seed,n_episodes,return,sem" > "$summary"

echo "============================================"
echo "Multi-Seed COMBO+DBG: Abiomed MCS (VAE)"
echo "seeds: ${seeds[*]}  |  device: $DEVICE  |  extra: ${EXTRA_ARGS:-none}"
echo "summary: $REPO/$summary"
echo "============================================"

[ -f "${GUARDIAN}_model.pth" ] && [ -f "${GUARDIAN}_meta_data.pkl" ] || {
    echo "ERROR: guardian not found: ${GUARDIAN}_model.pth / _meta_data.pkl" >&2; exit 1; }
...
for seed in "${seeds[@]}"; do
    ...
    # so this glob only ever matches VAE runs, further narrowed to this seed.
